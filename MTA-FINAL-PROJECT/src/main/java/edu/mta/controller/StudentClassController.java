...
import edu.mta.service.exel.StudentClassExcelService;
...
//			}
    ...
    public ResponseEntity<?> addNewStudentClass(@RequestBody String studentClassInfo) {
        ...
        try {
            ...
            if (!this.frequentlyUtils.checkKeysExist(jsonMap, "studentEmail", "classID")) {
                report = new ReportError(1, "Need to fill all required field");
                return ResponseEntity.badRequest().body(report);
            }
    ...
    @RequestMapping(path = "/downloadTemplateAddMultipleStudentToClass", method = RequestMethod.GET)
    @PreAuthorize("hasRole('ADMIN')")
    ...
    @RequestMapping(value = "/uploadFileToAddStudentsToClass", method = RequestMethod.POST)
    @PreAuthorize("hasRole('ADMIN')")
